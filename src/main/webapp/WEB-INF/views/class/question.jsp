...
<body>
    ...
    <div class="question-wrapper">
        <h2>문의 <button type="button" class="question-button btn btn-outline-dark"  data-bs-toggle="modal" data-bs-target="#questionModal">문의하기</button></h2>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.view-reply', function() {
        ...
        answerService.getList({question_no:questionNo},function (list) {
            ...
            // 답변 리스트 생성 후 입력창 생성
            if((user_id == owner_id) || (user_id == questionID)){        
    ...
    $(document).on("click", ".answer-remove", function(e) {
        ...
        if (confirmation) {
            answerService.remove({answer_no: answer_no, user_id: user_id}, function (result) {
                if (result == "success") {
                    console.log("result : ", result);
                    alert("삭제되었습니다.");
                    showQuestionList(pageNum); // 삭제 후, 질문 목록 갱신
